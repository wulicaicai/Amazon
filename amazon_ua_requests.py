...
def getOneWeekReview(urlReview, ASIN, reviewCount, review_date):
    ...
    if reEs != []:
        for ret in reEs:
            review_time = ret.replace(',', '').replace('on ', '').split(" ")
            review_time = review_time[2] + '-' + review_time[0] + '-' + (
                '0' + review_time[1] if int(review_time[1]) < 10 else review_time[1])
        ...
        reviewCount += 1;
        if callReviewDate >= last30day and len(reEs) == 10 and len(review_date) <= 25:
...
def getOneWeekQuestion(urlAnswered, ASIN, answerCount, questionDate):
    ...
    if reEs != []:
        ...
        answerCount += 1
        if callQuestionDate >= last30day and len(reEs) == 10 and len(questionDate) <= 25:
...
def GET_API():
    ...
    start_urls = []
    info = requests.get(
        url='http://testthird.gets.com:8383/api/index.php?act=getAmazonCubeinfo&sec=20171212getscn&site=1',
        timeout=30).text
...
# def infoParse():
    ...
    # for fullUrl in ['https://www.amazon.co.uk/AmazonBasics-Rooftop-Cargo-Carrier-litres/dp/B072ZHRDMZ/ref=zg_bsnr_automotive_6/258-2610431-8856422?_encoding=UTF8&psc=1&refRID=DXE3JJ7T7WBSR5GNE0AA']:
        ...
        # 抓取产品名称
        try:
            title = infoXpath.xpath('//span[@id="productTitle"]/font[@class]/font/text()| //span[@class="a-size-large"]/text()')[0].replace('\n', '').strip()
        except:
            continue
        # 抓取产品图片
        images = []
        ...
        if shop ==' ':
            shop = infoXpath.xpath('//div[@class="a-section a-spacing-none"]/a[@id="brand"]/@href')
            if shop != []:
                soplist=shop[0].split('=')
                shop = soplist[-1]
        ...
        tba = infoXpath.xpath(
            '//div[@class="a-section table-padding"]/table[@id="productDetails_detailBullets_sections1"]/tr')
        if tba:
            for tr in tba:
                Name = tr.xpath('./th/text()')[0].replace('\n', '').replace(' ', '')
                if Name == 'ItemWeight':  # 产品重量
                    ItemWeight = tr.xpath('./td/text()')
                    if ItemWeight != []:
                        ItemWeight[0].replace('\n', '').replace(' ', '').replace('(', '')
                    else:
                        ItemWeight = ' '
                if Name == 'ShippingWeight':  # 包装重量
                    ShippingWeight = tr.xpath('./td/text()')[0].replace('\n', '').replace(' ', '').replace('(', '')
                if Name == 'DateFirstAvailable' or Name == 'DatefirstlistedonAmazon':  # 上架时间
                    # 抓取上架时间
                    DateFirstAvailable = tr.xpath('./td/text()')
                    DateFirstAvailable = DateFirstAvailable[0].replace('\n', '').replace(' ', '')
                    times, DateFirstAvailable = dateTime(DateFirstAvailable, 1)
                if Name in ['Best0SellersRank', 'BestSellersRank']:  # 排名等信息
                    span_infore = re.compile(r"<span>\s+<span>(.*?) <a href='(.*?)'>(.*?)</a></span>\s+<br>\s+</span>")
                    span_info = span_infore.findall(info)
                    if span_info:
                        for info_ in span_info:
                            sListB = []
                            info = list(info_)
                            Best.append(info_)
                            sListB.append(info_[1])
                            Best1.append(sListB)
        if tba==[]:
            x = info.split('<div id="prodDetails" class="a-section">')[1]   # 蔡宇华 2019-4-18 此处xpath抓取到这里的时候，截取不到数据，原因不清楚，解决方法把info切表格那一块进行匹配
            infoXpath2 = etree.HTML(x)
            tba = infoXpath2.xpath(
                '//div[@class="a-section table-padding"]/table[@id="productDetails_detailBullets_sections1"]/tr')
            if tba:
                for tr in tba:
                    Name = tr.xpath('./th/text()')[0].replace('\n', '').replace(' ', '')
                    ...
                    if Name == 'DateFirstAvailable' or Name == 'DatefirstlistedonAmazon':  # 上架时间
                        ...
                        times, DateFirstAvailable = dateTime(DateFirstAvailable, 1)
                    if Name in ['Best0SellersRank', 'BestSellersRank']:  # 排名等信息
                        span_infore = re.compile(
                            r"<span>\s+<span>(.*?) <a href='(.*?)'>(.*?)</a></span>\s+<br>\s+</span>")
                        span_info = span_infore.findall(info)
                        if span_info:
                            for info_ in span_info:
                                sListB = []
        ...
        # 无表抓取
        if infoXpath.xpath(
                '//ul[@class="a-unordered-list a-nostyle a-vertical a-spacing-none"] | //div[@class="content"]/ul'):  # 验证一下table 里有ul 这个class吗
            # 上架时间
            DateFirstAvailable_re = re.compile(r"Date first listed on Amazon:\s+</span>\s+<span>(.*?)</span>")
            DateFirstAvailableList = DateFirstAvailable_re.findall(info)
            if DateFirstAvailableList:
                DateFirstAvailable = DateFirstAvailableList[0]
                times, DateFirstAvailable = dateTime(DateFirstAvailable, 2)

            # 包装重量
            ShippingWeight_re = re.compile(r"Shipping Weight:\s+</span>\s+<span>(.*?)\(<a.*?</span>")
            ShippingWeightList = ShippingWeight_re.findall(info)
            if ShippingWeightList != []:
                Shipping = ShippingWeightList[0]

            # 产品重量
            ItemWeight_re = re.compile(r"Item Weight:\s+</span>\s+<span>(.*?)\(<a.*?</span>")
            ItemWeightList = ItemWeight_re.findall(info)
            if ItemWeightList != []:
                Item = ItemWeightList[0]
