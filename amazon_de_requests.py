...
def dateTime(dateandtime, signal):
    print('dateTime')
    month = ['Januar', 'Februar', 'M', 'April', 'Mai', 'Juni', 'Juli', 'August', 'September', 'Oktober',
             'November', 'Dezember']
...
def getOneWeekReview(urlReview, ASIN, reviewCount, review_date):
    ...
    if reEs != []:
        for ret in reEs:
            ...
            review_time = review_time[2] + '-' + monChangeDe[review_time[1]] + '-' + (
...
def getOneWeekQuestion(urlAnswered, ASIN, answerCount, questionDate):
    ...
    if reEs != []:
        for ret in reEs:
            ...
            question_time = question_time[2] + '-' + monChangeDe[question_time[1]] + '-' + (
                '0' + question_time[0] if int(question_time[0]) < 10 else question_time[0])
...
def GET_API():
    ...
    start_urls = []
    info = requests.get(
        url='http://testthird.gets.com:8383/api/index.php?act=getAmazonCubeinfo&sec=20171212getscn&site=4',
        timeout=30).text
...
# def infoParse():
    ...
    # for fullUrl in ['https://www.amazon.de/Rucksack-Business-Schulrucksack-USB-Ladeanschluss-Wandern/dp/B07PMYY3D8/ref=zg_bsnr_luggage_3/262-2513439-5990932?_encoding=UTF8&refRID=Q7PEFGGRY5H8HA368R3M', 'https://www.amazon.de/Kartenetui-Kreditkarten-Kreditkartenetui-Geldb%C3%B6rse-Geldbeutel/dp/B07QHSB3NK/ref=zg_bsnr_luggage_4/262-2513439-5990932?_encoding=UTF8&refRID=Q7PEFGGRY5H8HA368R3M',]:
        ...
        if infoXpath.xpath(
            ...
            stars = infoXpath.xpath(
                '//div[@id="superLeafGameReviews_feature_div"]/div[@id="averageCustomerReviews"]/span[@class="a-declarative"]/span[@id="acrPopover"]/span[1]/a/i[1]/span/text()  | //div[@id="averageCustomerReviews_feature_div"]/div[@id="averageCustomerReviews"]/span[@class="a-declarative"]/span[@id="acrPopover"]/span[1]/a/i[1]/span/text()')[
                0]
            stars_re = re.compile(r'(.*?) von 5 Sternen')
        ...
        elif infoXpath.xpath('//div[@id="price"]/table/tr[1]/td[1]/text()') == 'Price:' or infoXpath.xpath(
                '//span[@id="priceblock_ourprice"]/text() | //span[@id="priceblock_dealprice"]/text()'):
            Price = infoXpath.xpath('//span[@id="priceblock_ourprice"]/text() | //span[@id="priceblock_dealprice"]/text()')[0]
            Price =  Price.replace(',','.')
            ListPrice = Price
        # 因为标签不一样,所以多出此方法进行处理
        ...
        seller_re = re.compile(
            r'<span class="olp-padding-right"><a href=".*?">(.*?)&nbsp;neu</a>&nbsp;ab&nbsp;<span class=.*?</span></span>')
        ...
        if tba:
            for tr in tba:
                ...
                if Name in ['Gewicht','Produktgewicht']:  # 产品重量
                    ItemWeight = tr.xpath('./td[2]/text()')
                    if ItemWeight != []:
                        ItemWeight=ItemWeight[0].replace('\n', '').replace(' ', '').replace('(', '')
                    else:
                        ItemWeight = ' '
                if Name == 'Produktgewichtinkl.Verpackung':  # 包装重量
                    ShippingWeight = tr.xpath('./td[2]/text()')[0].replace('\n', '').replace(' ', '').replace('(', '')
                if Name == 'ImAngebotvonAmazon.deseit' or Name == 'DatefirstlistedonAmazon':  # 上架时间
                    # 抓取上架时间
        ...
        # 无表抓取
        if infoXpath.xpath('//ul[@class="a-unordered-list a-nostyle a-vertical a-spacing-none"] | //td[@class="bucket"]/div[@class="content"]/ul'):  # 验证一下table 里有ul 这个class吗
            # 上架时间
            DateFirstAvailable_re = re.compile(r"Im Angebot von Amazon.de seit:</b> (.*?)</li>")
            ...
            if DateFirstAvailable:
                ...
                times = None
            # 包装重量
            ShippingWeight_re = re.compile(r'<li><b>Produktgewicht inkl\. Verpackung:</b> (.*?)</li>')
            ShippingWeight = ShippingWeight_re.findall(info)
            if ShippingWeight:
                ShippingWeight = ShippingWeight[0]

            # 产品重量
            ItemWeight_re = re.compile(r"Artikelgewicht:\s+</b>\s+(.*?)\s+</li>")
            ItemWeight = ItemWeight_re.findall(info)
            if ItemWeight:
                ItemWeight = ItemWeight[0]
        # 排名等信息
        # span_one = re.compile(r'<b>Amazon Bestseller-Rang:</b>\s+.*?<a href="(https:.*?)">.*?</a>\)')
        span_two = re.compile(
